...
public class MidiPlayer
{
	...
	public MidiPlayer()
	{
		...
		try{
	    	...
	    	// Workaround for JDK bug that causes java.util.prefs.WindowsPreferences <init>
			// warning when opening the synth device
			{
				...
				// This would normally raise the buggy warning, but now we're
				// temporarily swallowing all warnings
				synth.open();

				// Set stderr back to what it should be
				System.setErr(err);
			}
	...
	public void playNote(int note, int duration, int intensity)
	{
		try 
		{
			// It's possible for channel to be null (e.g., if the sound device is
	...
	// Additional methods for use in ObjectLand
